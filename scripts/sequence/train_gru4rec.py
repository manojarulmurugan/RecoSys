...
N_EPOCHS        = 30
EVAL_EVERY      = 5
TEMPERATURE     = 0.20  # T=0.07 caused over-personalisation: model peaked at ep5 (R@10=0.0535)
                        # then degraded — below Global Popularity (0.0646).  T=0.20 keeps
                        # strong gradients while preserving popularity signal in embeddings.
NUM_WORKERS     = 4
GRAD_CLIP       = 1.0
...
def main() -> None:
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        if epoch % EVAL_EVERY == 0:
            ...
            # Save best checkpoint by R@10.
            if ep_metrics["recall_10"] > best_r10:
                best_r10   = ep_metrics["recall_10"]
                best_epoch = epoch
                import shutil
                shutil.copy2(ckpt_path, CHECKPOINT_DIR / "best_recall10.pt")
