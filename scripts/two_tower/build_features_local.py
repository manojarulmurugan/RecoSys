...
builder = FeatureBuilder(
    interactions_path  = "gs://recosys-data-bucket/samples/users_sample_50k/interactions/",
    item_features_path = "gs://recosys-data-bucket/features/item_features/",
    user_features_path = "gs://recosys-data-bucket/features/user_features_50k/",
    output_dir         = str(_REPO_ROOT / "artifacts" / "50k"),
)
...
# Check 11 — users_sample_50k interactions are ~750k rows (not 1–2M); bounds allow that sample
def _c11():
    n = len(_train_pairs)
    assert 400_000 <= n <= 2_000_000, \
        f"train_pairs row count {n:,} not in [400_000, 2_000_000]"
check(11, "train_pairs row count in expected range (50k-user sample)", _c11)
...
# Check 14
def _c14():
    n_vocab  = len(_vocabs["user2idx"])
    n_unique = _users_enc["user_idx"].nunique()
    assert n_unique == n_vocab, \
        f"users_encoded has {n_unique} unique user_idx but vocab has {n_vocab}"
check(14, "users_encoded.user_idx matches vocab size", _c14)
