...
DATASET = "recosys"
GCS_URI = "gs://recosys-data-bucket/features/user_features_50k/*.parquet"
...
# ── 1. Create user_features_50k table ─────────────────────────────────────────
print("Building user_features_50k table in BigQuery...")

create_sql = f"""
CREATE OR REPLACE TABLE `{PROJECT}.{DATASET}.user_features_50k` AS
WITH cat_ranked AS (
  SELECT
    t.user_id,
    f.cat_l1,
    ROW_NUMBER() OVER (PARTITION BY t.user_id ORDER BY COUNT(*) DESC) AS rn
  FROM `{PROJECT}.{DATASET}.train_50k` t
...
FROM `{PROJECT}.{DATASET}.train_50k` b
...
job.result()
print("[ok] user_features_50k table created")

# ── 2. Validate row count ──────────────────────────────────────────────────────
count_sql = f"SELECT COUNT(*) AS n FROM `{PROJECT}.{DATASET}.user_features_50k`"
n = list(client.query(count_sql).result())[0].n
print(f"  Row count: {n:,}  (expected: 44,559)")
assert 40_000 < n < 50_000, f"Unexpected row count: {n}"
...
FROM `{PROJECT}.{DATASET}.user_features_50k`
"""
...
SELECT * FROM `{PROJECT}.{DATASET}.user_features_50k`
ORDER BY user_id
"""

job = client.query(export_sql)
job.result()
print("[ok] user_features_50k exported to GCS")
print("\nDone. Artifacts:")
print(f"  BQ  : {PROJECT}.{DATASET}.user_features_50k")
print(f"  GCS : {GCS_URI}")